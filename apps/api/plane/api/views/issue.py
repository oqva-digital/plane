...
class IssueDetailAPIEndpoint(BaseAPIView):
        ...
        if external_id and external_source:
            try:
                ...
                if serializer.is_valid():
                    ...
                    # Dispatch webhook for work-item update (same as frontend path)
                    model_activity.delay(
                        model_name="issue",
                ...
                if serializer.is_valid():
                    ...
                    # Dispatch webhook for work-item create (same as POST path)
                    model_activity.delay(
                        model_name="issue",
                        model_id=str(serializer.data["id"]),
                        requested_data=request.data,
    ...
    def patch(self, request, slug, project_id, pk):
        ...
        if serializer.is_valid():
            ...
            # Dispatch webhook for work-item partial update (same as frontend path)
            model_activity.delay(
                model_name="issue",
                model_id=str(pk),
                requested_data=request.data,
